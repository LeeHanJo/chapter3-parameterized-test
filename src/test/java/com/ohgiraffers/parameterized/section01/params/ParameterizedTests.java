...
public class ParameterizedTests {
    ...
    * parameterized-test 는 @Parameterized 어노테이션을 @Test 어노테이션을 대체하여 사용하며,
    * 이 경우 테스트 메소드에 매개변수를 선언할 수 있다. (일반적인 테스트에서는 매개변수를 사용 할 수 없다.
    * 파라미터로 전달할 값의 목 만큼 반복적으로 테스트 메소드를 실행하여, 반복 실행 시 마다 준비된 값 목록을
    * 매개변수로 전달하여 테스트 코드를 실행하게 된다.
    * given when then 패턴에서사전에 테스트를 위해 준비하던 ,given 부분을 대체할 수 있다.*/
    ...
    * 필기
    * 기본 자료형 타입은 null 이나 빈 값을 파라미터로 전달하기 위해 사용되는 어노테이션이다.
    * null 값과 빈 값을 모두 전달하기 위해 구성된 @NullAndEmptySource 도 이용 할 수 있다.*/
    ...
    *  필기. 열거형에 작성된 타입들을 파라미터로 전달하여 테스트에 활용할 수 있다.*/
    ...
    @DisplayName("Month에 정의돤 타입들이 1~12월 사이의 범위인지 테스트")
    @ParameterizedTest
...
//    @CsvSource({"test, TEST", "tEsT, TEST", "JavaScript, JAVASCRIPT"})
    ...
    @DisplayName("CSV 파일을 읽은 테스트 데이터를 데스트에 활용하는지 확인")
    @ParameterizedTest
    @CsvFileSource(resources = "/parameter-test-data.csv")
